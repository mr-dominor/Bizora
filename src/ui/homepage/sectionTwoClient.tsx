...
export default function SectionTwoClient({ data }: { data: Item[] }) {
   ...
   const scrollNext = useCallback(() => emblaApi?.scrollNext(), [emblaApi])
    return (
    <motion.div
      className="w-full h-[450px] bg-white px-2 md:px-10 text-black "
      initial={{ opacity: 0, y: 100 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.8, ease: 'easeOut' }}
      viewport={{ once: true, amount: 0.5 }}
    >
      <h1 className='text-2xl font-geistMono py-2'>Shop By Fabric</h1>
        ...
        <div className="flex">
          {data.map((item, index) => (
            <div className="w-96 h-[350px] px-10 py-2 relative aspect-video" key={index}>
              <Link href={`/shop/fabric/${item.fabric}`}>
              ...
              />
                ...
                <h1 className={`${quicksand.className} font-medium text-center sm:text-left text-lg`}>{item.fabric}</h1>
